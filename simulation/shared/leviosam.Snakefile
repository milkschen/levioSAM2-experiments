...
# rule bt2_collate:
#     input:
#         commit = os.path.join(DIR_FIRST_PASS, f'bt2-{SOURCE_LABEL}_to_{TARGET_LABEL}-committed.bam'),
#         defer = os.path.join(DIR_FIRST_PASS, f'bt2-{SOURCE_LABEL}_to_{TARGET_LABEL}-deferred.bam'),
#     output:
#         commit = os.path.join(DIR_FIRST_PASS, f'bt2-{SOURCE_LABEL}_to_{TARGET_LABEL}-paired-committed.bam'),
#         defer = temp(os.path.join(DIR_FIRST_PASS, f'bt2-{SOURCE_LABEL}_to_{TARGET_LABEL}-paired-deferred.bam')),
#         fq1 = temp(os.path.join(DIR_FIRST_PASS, f'bt2-{SOURCE_LABEL}_to_{TARGET_LABEL}-paired-deferred-R1.fq.gz')),
#         fq2 = temp(os.path.join(DIR_FIRST_PASS, f'bt2-{SOURCE_LABEL}_to_{TARGET_LABEL}-paired-deferred-R2.fq.gz')),
#     params:
#         prefix = os.path.join(DIR_FIRST_PASS, f'bt2-{SOURCE_LABEL}_to_{TARGET_LABEL}-paired'),
#     shell:
#         '{LEVIOSAM} collate -a {input.commit} -b {input.defer} -p {params.prefix}'
# 
# rule bt2_sort_defer:
#     input:
#         bam = os.path.join(DIR_FIRST_PASS, f'bt2-{SOURCE_LABEL}_to_{TARGET_LABEL}-paired-deferred.bam'),
#     output:
#         bam = os.path.join(DIR_FIRST_PASS, f'bt2-{SOURCE_LABEL}_to_{TARGET_LABEL}-paired-deferred-sorted_n.bam'),
...
#     input:
#         defer = os.path.join(DIR_FIRST_PASS, f'bt2-{SOURCE_LABEL}_to_{TARGET_LABEL}-paired-deferred-sorted_n.bam'),
#         realn = os.path.join(DIR_FIRST_PASS, f'bt2-{SOURCE_LABEL}_to_{TARGET_LABEL}-paired-realigned-sorted_n.bam')
#     output:
#         bam = os.path.join(DIR_FIRST_PASS, f'bt2-{SOURCE_LABEL}_to_{TARGET_LABEL}-paired-deferred-cherry_picked.bam')
#     shell:
...
#     input:
#         commit = os.path.join(DIR_FIRST_PASS, f'bt2-{SOURCE_LABEL}_to_{TARGET_LABEL}-paired-committed.bam'),
#         cherry_picked = os.path.join(DIR_FIRST_PASS, f'bt2-{SOURCE_LABEL}_to_{TARGET_LABEL}-paired-deferred-cherry_picked.bam')
#     output:
#         bam = os.path.join(DIR_FIRST_PASS, f'bt2-{SOURCE_LABEL}_to_{TARGET_LABEL}-final.bam')
#     threads: int(THREADS * 0.4)
#     shell:
#         '{SAMTOOLS} cat {input.commit} {input.cherry_picked} | '
#         '{SAMTOOLS} sort -@ {threads} -o {output.bam}'

rule bwa_lift_source_to_target:
    input:
